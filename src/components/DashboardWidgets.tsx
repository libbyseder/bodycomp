...
export default function DashboardWidgets({ measurements }: DashboardWidgetsProps) {
  ...
  const weightProgress = weightGoal
    ? currentWeight <= weightGoal
      ? 100
      : Math.max(0, Math.min(100, 100 - ((currentWeight - weightGoal) / currentWeight) * 100))
    : 0

  const bfProgress = bfGoal
    ? currentBf <= bfGoal
      ? 100
      : Math.max(0, Math.min(100, 100 - ((currentBf - bfGoal) / currentBf) * 100))
    : 0

  const ffmiProgress = ffmiGoal && currentFfmi
    ? currentFfmi >= ffmiGoal
      ? 100
      : Math.max(0, Math.min(100, (currentFfmi / ffmiGoal) * 100))
    : 0
